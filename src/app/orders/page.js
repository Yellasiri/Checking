...
//                   <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="absolute inset-0 opacity-0 cursor-pointer rounded-full">
...
//                               <span className={`${isGradient ? "text-white" : "text-[#02143F]"} text-[12px] font-semibold`}>Order ID: {String(o._id).slice(-6).toUpperCase()} {orderStatus[o._id]}</span>
...
//                                 <IoIosArrowDown className={`${isGradient ? "text-white" : "text-[#02143F]"}`} />
//                                 <select aria-label="Change status" className="absolute inset-0 opacity-0 cursor-pointer" value={st} onChange={(e) => handleStatusChange(o._id, e.target.value, st)}>
//                                   {STATUS.filter((s) => s !== "All").map((s) => (<option key={s} value={s}>{s}</option>))}
//                                 </select>
//                               </div>
//                             </div>
...
//           {/* Imported SendImagesModal used in two modes:
//                - showSendModal + selectedOrder => send images for order
//                - showGalleryForAdd => choose images for Add Order design (first selected used)
//           */}
//           <SendImagesModal
